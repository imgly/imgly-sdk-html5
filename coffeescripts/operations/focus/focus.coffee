...
class Focus extends Operation
  ...
  apply: (imageData) ->
    # Only blur once, this increases performance
    fingerprint = Utils.fingerprint(imageData.data)
    unless fingerprint is @fingerprint
      @cache = @blur Utils.cloneImageData(imageData)
      @fingerprint = fingerprint

    maskImageData = @createMaskImageData imageData

    for index in [0...imageData.data.length] by 4
      alpha = maskImageData.data[index] / 255

      imageData.data[index]     = alpha * imageData.data[index]     + (1 - alpha) * @cache.data[index]
      imageData.data[index + 1] = alpha * imageData.data[index + 1] + (1 - alpha) * @cache.data[index + 1]
      imageData.data[index + 2] = alpha * imageData.data[index + 2] + (1 - alpha) * @cache.data[index + 2]
  ...
  ###
    ...
    @returns {ImageData}
  ###
  createMaskImageData: (originalImageData) ->
  ...
  blur: (imageData) ->
    ...
    for i in [1...div]
      ...
      if i is radiusPlus1 then stackEnd = stack
